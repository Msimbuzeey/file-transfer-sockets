...
def send_file(tx_file: Path, client_socket: socket.socket, tx_file_name: str = None):
    ...
    client_socket.sendall(
        bytes(f"{FILE_START_SEND_FLAG}{tx_file_name}{FILE_METADATA_SEPARATOR}{tx_file_size}", "utf-8"))

    with open(tx_file, "rb") as tx_file:
        print(f"Sending file: [{tx_file_name}]")
        try:
            total_bytes_tx = client_socket.sendfile(tx_file)
            # client_socket.sendall(FILE_END_FLAG)
            print(f"DONE: Sent {total_bytes_tx} bytes")
        except Exception as e:
            print(f"ERROR: {e}, SENT: {tx_file.tell()} bytes only")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        with c_skt:
            while True:
                ...
                # If the provided path is a file, then just send normally
                ...
                #   Didn't do this, because that would require using zipfile library and extra logic outside scope of this
                #   script, but feel free to do so in your implementation
                #   2) Recursively iterate through sub dirs, using either os.walk or this method below
